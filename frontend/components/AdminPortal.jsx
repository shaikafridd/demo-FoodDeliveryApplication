...
export default function AdminPortal({ onLogout }) {
  const [activeTab, setActiveTab] = useState('orders'); // 'orders' | 'payments' | 'outlets' | 'menu'
  ...
  const loadData = async () => {
    ...
    if (dbOrders && dbOrders.length > 0) {
      setOrders(dbOrders.map(o => ({
        ...
        items: Array.isArray(o.items) ? o.items : [o.items || '1x Item'],
    ...
    // 2. Fetch Payments
    const storedPayments = getLocalStoreData('payments', [
  ...
  const handleCreateOrderSubmit = async (e) => {
    ...
    const formattedOrder = {
      ...
      items: Array.isArray(created.items) ? created.items : [created.items],
    ...
    saveLocalStoreData('orders', updatedOrders);

    // Also record UPI payment
    const newPay = {
      id: `pay-${Date.now()}`,
    ...
    saveLocalStoreData('payments', updatedPayments);

    setShowAddOrderModal(false);
    setNewOrderData({ customerName: '', customerPhone: '', items: '', totalAmount: '', orderStatus: 'Received' });
    showToast(`New order #${formattedOrder.orderNumber} created!`);
  };
  ...
  const handleAddOutletSubmit = async (e) => {
    ...
    await registerRestaurantApi(outletObj);

    const updated = [outletObj, ...outlets];
    setOutlets(updated);
    ...
    showToast(`Outlet "${outletObj.name}" added successfully!`);
  };

  // Toggle Outlet Status
  const handleToggleOutletStatus = (outletId) => {
    const updated = outlets.map(o => {
      if (o.id === outletId || o._id === outletId) {
    ...
    setOutlets(updated);
    saveLocalStoreData('outlets', updated);
    showToast('Outlet status updated');
  };
  ...
  const handleAddMenuItemSubmit = (e) => {
    ...
    showToast(`Menu Item "${newItem.name}" added!`);
  };

  const handleEditMenuItemSubmit = (e) => {
    e.preventDefault();
    if (!showEditMenuModal) return;

    const updated = menuItems.map(item => {
      if (item.id === showEditMenuModal.id || item._id === showEditMenuModal.id || item._id === showEditMenuModal._id) {
        return {
          ...item,
          name: menuFormData.name,
          category: menuFormData.category,
  ...
  const handleToggleMenuStock = (itemId) => {
    const updated = menuItems.map(i => {
      if (i.id === itemId || i._id === itemId) {
        return { ...i, inStock: i.inStock === false ? true : false };
      }
      return i;
    });
    setMenuItems(updated);
      ...
      <header className="admin-header">
        <div className="admin-header-brand">
          ...
          <div>
            ...
            <span className="admin-subtitle">Store Operations • Direct Orders & UPI Settlement Control</span>
          </div>
        </div>
                    ...
                    <tr key={order.id}>
                      ...
                      <td>
                        <div className="table-actions-cell">
                          ...
                          <button className="btn-icon danger" onClick={() => handleDeleteOrder(order.id)} title="Delete Order">
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </td>
            ...
            <table className="admin-data-table">
              ...
              <tbody>
                {outlets
                  ...
                  .map((outlet, idx) => {
                    const id = outlet.id || outlet._id || idx;
          ...
          <div className="modal-card">
            ...
            <form onSubmit={handleAddOutletSubmit} className="modal-form">
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-primary">Onboard Outlet</button>
              </div>
            </form>
          </div>
        </div>
